...
// ── Dictate button ────────────────────────────────────────────
// Doesn't do any speech-to-text itself — there's no web API that can
// reach into iOS/Android and press the keyboard's own mic button, and
// building a from-scratch transcription flow (Web Speech API) means
// depending on network mid-job (exactly when signal is worst) and
// spotty iOS Safari support across a mixed-device crew. What this
// actually does: jump straight to "keyboard up, cursor in the right
// spot," which is the fiddly part on a small phone screen — the phone's
// own keyboard mic (already on every iOS/Android keyboard) does the
// rest. Cursor is placed at the very end of any existing text, with a
// separating space inserted first if there's already content, so
// whatever gets dictated next appends onto prior notes instead of
// landing in the middle of a sentence or overwriting a selection.
function focusForDictation(fieldId) {
  const el = document.getElementById(fieldId);
  if (!el) return;

  if (el.value && !/\s$/.test(el.value)) el.value += ' ';
  el.focus();
  el.setSelectionRange(el.value.length, el.value.length);
  el.scrollTop = el.scrollHeight;

  // One-time nudge toward the keyboard's mic icon — after that, the
  // button itself (icon + "Dictate" label) is reminder enough.
  if (!localStorage.getItem('mg_dictate_tip_seen')) {
    localStorage.setItem('mg_dictate_tip_seen', '1');
    showToast('Now tap the 🎤 mic icon on your keyboard to talk', 4000);
  }
}
